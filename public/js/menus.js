$(function() {
  // 添加、修改菜单弹框html
  var getMenusPopHtml = function(data) {
    ...
    var html = '<div class="page-pop">';
        ...
        html += '</div>';
        html += '<div class="form-group">';
        html += '<label for="name" class="col-md-3 control-label">菜单名称：</label>';
        html += '<div class="col-md-7"><input type="text" class="form-control" name="name" value="'+ name +'" id="name"></div>';
        html += '</div>';
        html += '<div class="form-group">';
        html += '<label for="url" class="col-md-3 control-label">菜单链接：</label>';
  ...
  $('#addBtn').on('click', function () {
    layer.open({
      type: 1,
      title: '添加菜单',
      content: '<div id="addMenus">'+ getMenusPopHtml() +'</div>',
      area: ['420px', '280px']
    });
    var $addMenus = $('#addMenus');
    var $form = $addMenus.find('form');
    ...
    $form.submit(function () {
      ...
      if (!$name.val()) {
        Layer.msg_lose('菜单名称不能为空！');
        $name.focus();
        return false;
      }
      if (!$url.val()) {
        Layer.msg_lose('菜单链接不能为空！');
        $url.focus();
        return false;
      }
      $.ajax({
      ...
      success: function(result) {
        if (!result.code) {
          layer.open({
            type: 1,
            title: '编辑菜单',
            content: '<div id="editMenus">'+ getMenusPopHtml(result.data) +'</div>',
            area: ['420px', '280px']
          });
          var $editMenus = $('#editMenus');
          var $form = $editMenus.find('form');
          var $order = $form.find('input[name="order"]');
          var $name = $form.find('input[name="name"]');
          var $url = $form.find('input[name="url"]');
          $form.submit(function () {
